@extends('frontend::layouts.master', ['title' => 'Donasi Sekarang'])
@section('content')
    <div class="container-fluid py-5">
        <div id="wizard">
            ...
            <section>
                ...
                <h2 class="section-heading">Masukan Nominal Donasi</h2>
        ...
        $("#wizard").steps({
            ...
            onStepChanging: function(event, currentIndex, newIndex) {
                ...
                if ($('#show-hamba-allah').is(':checked')) {
                    ...
                    if (currentIndex == 1) {
                        $.ajax({
                            url: '{{ route('frontend.donasi.payment', ':id') }}'.replace(':id',
                                '{{ $donasi->id }}'),
                            method: 'post',
                            data: {
                                _token: '{{ csrf_token() }}',
                                    ...
                                    window.snap.embed(response.data, {
                                        ...
                                        onSuccess: function(result) {
                                            window.location.replace(
                                                `{{ route('payment.handle') }}/?order_id=${result.order_id}&transaction_status=${result.transaction_status}&donasi_id={{ $donasi->id }}`
                                            );
                                        },
